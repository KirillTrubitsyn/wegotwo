...
function buildSystemInstruction(ctx: TripContext): string {
  ...
  return [
    ...
    "  When an electronic ticket contains more than one flight segment",
    "  (e.g. SVO → BEG → TIV on the way out, TIV → BEG → SVO on return),",
    "  fill the `segments` array IN BOARDING ORDER. Each segment keeps its",
    "  own airline, flight code, from/to codes and cities, dep_at/arr_at,",
    "  seat, terminal, baggage. Also fill the top-level flight fields with",
    "  the FIRST segment's data (so old code that does not read segments",
    "  still sees the initial leg). For the top-level `code` you may",
    "  concatenate all flight numbers comma-separated. For single-segment",
    "  tickets leave `segments` empty and fill the top-level fields only.",
    "",
