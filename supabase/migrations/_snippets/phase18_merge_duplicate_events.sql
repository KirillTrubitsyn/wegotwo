...
--      заполненных полей (description, photo_path, tour_details,
--      map_url, start_at); при равенстве — более ранняя.
--   3. Coalesce non-null значения из loser'ов в survivor.
--   4. Attachments объединяем как union по document_id.
--   5. Loser'ов удаляем.
--
-- ВАЖНО: реализовано чистым SQL с временной таблицей, без
-- PL/pgSQL DO-блока, потому что имя локальной переменной `trip_uuid`
-- в plpgsql начинало конфликтовать с SQL-парсером внутри CTE
-- (ERROR 42P01: relation "trip_uuid" does not exist).
--
